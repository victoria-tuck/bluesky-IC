...
case_file_path = "test_cases/case3.json"
...
methods = ["ff", "ascending-auction-profitbased","ascending-auction-budgetbased"]
if not Path(result_path).is_file(): # won't rerun auctions if pickle exists (set to true for testing)
    ...
    for method in methods:
        for lambda_val in LAMBDAS:
            ...
            _, scenario_result = run_scenario(test_case_data, "", "", case_file_path, method, design_parameters=design_parameters, save_scenario=False, payment_calc=False)
...
print(f"ASC AUC (profit-based) Valuations : {valuations[1]}, Congestion costs: {congestion_costs[1]}")
print(f"ASC AUC (budget-based) Valuations : {valuations[2]}, Congestion costs: {congestion_costs[2]}")
print(f"FF SW: {[valuations[0][i] - congestion_costs[0][i] for i in range(len(LAMBDAS))]}")
print(f"ASC AUC (profit-based) SW: {[valuations[1][i] - congestion_costs[1][i] for i in range(len(LAMBDAS))]}")
print(f"ASC AUC (budget-based) SW: {[valuations[2][i] - congestion_costs[2][i] for i in range(len(LAMBDAS))]}")
...
plt.scatter(plotted_congestion[1], valuations[1], sizes=sizes, color="green")
plt.scatter(plotted_congestion[2], valuations[2], sizes=sizes, color="red")

plt.plot(plotted_congestion[0], valuations[0], "--", color = "blue", label = "FF")
plt.plot(plotted_congestion[1], valuations[1], "--", color = "green", label = "Ascending Auction (profit-based)")
plt.plot(plotted_congestion[2], valuations[2], "--", color = "red", label = "Ascending Auction (budget-based)")
...
print(plotted_congestion)
plt.annotate("all", (plotted_congestion[0][len(LAMBDAS)-1] + 10 , valuations[0][len(LAMBDAS)-1] + 10))
plt.annotate("all", (plotted_congestion[1][len(LAMBDAS)-1] + 10 , valuations[1][len(LAMBDAS)-1] + 10))
plt.annotate("all", (plotted_congestion[2][len(LAMBDAS)-1] + 10 , valuations[2][len(LAMBDAS)-1] + 10))
